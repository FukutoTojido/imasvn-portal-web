				...
				{days.map((day) => {
					return (
						<button
							...
							className={`
								text-left relative rounded-md p-2.5 text-sm flex flex-col gap-2.5 justify-between font-bold hover:bg-text hover:text-mantle hover:opacity-100 transition-all
								${day.date.getMonth() !== month ? "opacity-40" : ""} 
								${compareDate(day.date, currentDate) ? "bg-text text-mantle" : "text-text bg-overlay-0/20"}
								`.replaceAll(/\s+/g, " ")}
							onClick={() => {
								setIdols?.(
						...
						>
							<div
								className={`absolute top-0 left-0 w-full h-full rounded-md ${compareDate(day.date, currDate) ? "border-2 border-text" : ""}`}
							/>
							{day.date.getDate()}
